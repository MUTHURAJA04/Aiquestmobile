...
const plans = [
  {
    title: 'Free Trial',
    price: '₹0',
    subtitle: '3 Free Attempts • Basic Access',
    features: ['3 quiz attempts', '5 generated questions', 'Basic performance tracking', 'Community support'],
    cta: 'Start Free Trial',
    gradient: ['#dbeafe', '#93c5fd'], // Light gray gradient
  },
  {
    title: 'Basic',
    price: '₹499',
    ...
    features: ['Unlimited AI quiz generation', '20 questions per day', 'Advanced analytics', 'Email support'],
    cta: 'Subscribe Now',
    gradient: ['#dbeafe', '#93c5fd'], // Light blue gradient
  },
  {
    title: 'Standard',
    price: '₹1199',
    ...
    features: ['Unlimited AI quiz generation', '50 questions per day', 'Priority support', 'Progress reports'],
    cta: 'Buy Now',
    popular: true,
    gradient: ['#dbeafe', '#93c5fd'], // Purple gradient
  },
  {
    title: 'Premium',
    price: '₹2199',
    period: '/ 6 Months',
    subtitle: 'Best Value',
    features: ['Unlimited AI quiz generation', '100 questions per day', '24/7 support', 'Custom quiz templates'],
    cta: 'Buy Now',
    gradient: ['#dbeafe', '#93c5fd'], // Green gradient
  },
  {
    title: 'Elite',
    price: '₹4199',
    ...
    features: ['Unlimited everything', 'API access', 'Dedicated account manager', 'White-label options'],
    cta: 'Contact Sales',
    gradient: ['#dbeafe', '#93c5fd'], // Yellow gradient
  },
];
  ...
  return (
    <ScrollView className="bg-gray-50">
        ...
        <View className="absolute inset-0 opacity-20 bg-black">
          <Image source={require('../assets/Feature.jpg')} className="w-full h-full" resizeMode="cover" />
        </View>
      ...
      <View className="px-4 py-8">
        ...
        <View className="flex-row flex-wrap justify-center" style={{gap: 16}}>
          {plans.map((plan, index) => (
            <View 
              key={index} 
              className={`rounded-2xl overflow-hidden w-full max-w-md ${plan.popular ? 'border-2 border-blue-300' : 'border border-gray-200'}`}
              style={{
                shadowColor: '#000',
            ...
            >
              ...
              >
                ...
                <View className="flex-row items-end mt-2">
                  ...
                  <Text className="text-gray-600 ml-1 mb-1">{plan.period}</Text>
                </View>
                <Text className="text-gray-700 mb-4">{plan.subtitle}</Text>
                ...
                <TouchableOpacity 
                  className={`mt-6 rounded-xl py-3 ${plan.popular ? 'bg-blue-600' : 'bg-blue-500'}`}
                  activeOpacity={0.8}
                >
                  <Text className="text-center text-white font-semibold text-lg">{plan.cta}</Text>
                </TouchableOpacity>
              </LinearGradient>
            </View>
          ))}
  ...
  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
    <View className="rounded-xl border border-gray-300 overflow-hidden min-w-[700px]">
      ...
      <View className="flex-row bg-gray-200 border-b border-gray-300">
        {['Features', 'Free', 'Basic', 'Premium'].map((header, i) => (
          <View
            key={i}
            className={`p-4 ${i === 0 ? 'w-2/5' : 'w-1/5'} justify-center items-center`}
          >
            <Text className="text-base font-semibold text-gray-800 text-center">{header}</Text>
          </View>
        ))}
      </View>
      ...
      {[
        ...
        >
          {row.map((cell, cellIndex) => (
            <View
              key={cellIndex}
              className={`${cellIndex === 0 ? 'w-2/5' : 'w-1/5'} p-4 justify-center items-center`}
            >
              <Text className="text-base text-gray-700 text-center">{cell}</Text>
            </View>
          ))}
        </View>
      ))}
    ...
    {faqs.map((faq, i) => (
      ...
      >
        ...
        >
          ...
          <Icon
            name={openFaqIndex === i ? 'minus' : 'plus'} // ✨ only plus/minus
            size={24}
            color="#3b82f6"
          />
        </TouchableOpacity>
