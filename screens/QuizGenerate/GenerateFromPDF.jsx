...
const GenerateFromPDF = () => {
    ...
    try {
      ...
      if (result && result.length > 0) {
        ...
        if (selectedFile.size > 50 * 1024 * 1024) {
          Alert.alert(
            'Large File',
            'This PDF is large. Generating questions may take longer.',
            [{ text: 'OK' }]
          );
        }
  ...
  const validateInputs = () => {
    if (!file) {
      Alert.alert('Missing File', 'Please upload a PDF file first.');
      return false;
    }
    if (!questionType) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (error.message.includes('timeout') || error.message.includes('Timeout')) {
        errorMessage += 'The file is too large or taking too long. Try a smaller PDF file.';
      ...
      <ScrollView className="px-6 py-8" contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        >
          ...
          <Text className="text-blue-100 text-center mt-2 text-sm">
            Upload a PDF file and we'll create questions for you
          </Text>
        </LinearGradient>

        <View className="mb-6">
          <Text className="text-lg font-semibold text-gray-800 mb-3">PDF File</Text>
          <TouchableOpacity
            onPress={pickPDF}
            disabled={loading}
