...
const QuizAnswer = () => {
  ...
  const handleSubmit = async (auto = false) => {
    ...
    try {
      // 🧠 Format user answers for backend
      const userAnswers = {};
      quizData.questions.forEach((q, index) => {
        const selected = selectedAnswers[index];
        const answerIndex = q.options.findIndex((opt) => opt === selected);
        userAnswers[q._id || q.id || index] = answerIndex;
      });
      const res = await submitQuiz(
        quizData.quiz_id || quizData._id || quizData.id,
        userAnswers
      );
      ...
      if (auto) {
        ...
        Alert.alert("✅ Submitted", "You submitted the quiz successfully!");
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        <Text className="text-3xl font-bold text-center mb-6 text-indigo-700">
          AI Quiz
        </Text>
        ...
        {isSubmitted && (
          <View className="items-center mb-6">
            ...
            <Text className="text-sm mt-2 text-blue-600 font-medium">
              {submittedOnTime
                ? "⏳ You submitted on time. Great focus!"
                : "⏱ Submitted automatically when time expired."}
            </Text>
          </View>
        )}
