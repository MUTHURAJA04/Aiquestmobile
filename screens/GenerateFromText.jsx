...
const GenerateFromText = () => {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      /*  Payload names MUST match backend  */
      const payload = {
        text,
        question_type    : questionType,          // "mcq" | "boolean"
        number_questions : Number(numberOfQuestions), // send as number, not string
        difficulty,
        token,
      };
    ...
    >
      <ScrollView className="px-6 py-8" contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        {loading ? (
          ...
          <LinearGradient colors={['#3b82f6', '#33aaea']} style={{ borderRadius: 12, padding: 16, marginBottom: 24 }}>
          <TouchableOpacity
